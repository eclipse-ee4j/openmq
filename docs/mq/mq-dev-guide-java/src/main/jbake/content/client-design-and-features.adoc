...
must be created using the `javax.jms` package.
...
defined as `javax.jms`. All loggers in the Message Queue client runtime
...
runtime are logged by the logger with the `javax.jms` name space.
...
space `javax.jms` at level `FINER`.
...
`javax.jms.connection` name space and specifies that output be written
...
events by changing the `javax.jms.connection` name space log level to
