...
package `javax.jms`) and as a Message Queue class (in
...
package `javax.jms.*`, you must qualify the constructor with the full
...
rather than to the `ConnectionFactory` interface defined in `javax.jms`
...
`ConnectionFactory` interface, defined in package `javax.jms`; the
...
constants in the standard JMS `Session` interface, `javax.jms.Session` ;
...
standard JMS interface `javax.jms.Destination` (which the Message Queue
class implements) is sufficient. If you have imported the full JMS
package `javax.jms.*`, you can simply declare the variable with the
...
The standard JMS `Session` interface, defined in package `javax.jms`,
