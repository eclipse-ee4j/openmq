...
has `javax.jms.Message` as its formal type parameters. To specify a
...
1.  [[CHDHHEAD]] +
The bridge creates a new DMQ `javax.jms.ObjectMessage` object and sets
...
3.  The body of the `javax.jms.ObjectMessage` is set to the transformed
...
4.  [[CHDGGAHA]] +
The `javax.jms.ObjectMessage` is sent (up to `send-attempts` times) to
...
source and target must be `javax.jms.XAConnectionFactory` objects.
...
`javax.jms.ConnectionFactory` objects.
...
`javax.jms.Message.getJMSDestination()`, unless overridden by the
...
`javax.jms.ConnectionFactory` object.
...
`javax.jms.ConnectionFactory` or `javax.jms.XAConnectionFactory` type
...
`javax.jms.Message`). Place this class under the `IMQ_HOME/lib/ext`
directory.
